...
class CriminalOtherService
{
     ...
     public function update($request)
    {
        ...
        if ($request->hasFile('incidentevedance')) {
            $file = $request->file('incidentevedance');
            $currentDate = now()->format('Ymd');
            $renamedFileName = "{$currentDate}_{$request->recordID}_4_{$request->arretedcrime}." . $file->getClientOriginalExtension();
            $file->storeAs('Evedances', $renamedFileName, 'public');

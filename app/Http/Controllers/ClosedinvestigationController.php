...
class ClosedinvestigationController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function($row) {
                    ...
                    if ($row->approve_status == 0 ) {
                        if (auth()->user()->can('Closed-Investigation-Approve')) {
                    ...
                    if (auth()->user()->can('Closed-Investigation-Print')) {
                    ...
                    $btn .= '<a href="' . route('closedinvestigationsview', ['id' => $row->id]) . '"  target="_self" title="Approve" data-bs-toggle="tooltip" data-bs-placement="top"  class="icon-button btn btn-info btn-sm mr-1 editbtn"><i class="material-icons">visibility</i></</a>';
    ...
    public function view($investigationID)
    {
     ...
     return view('Investigations.Closed_Investigation.closedinvestigationview', compact('policedivisions','maincrimecategory','officers',
        'stations','investigationinfo','victims','divisionID','crimelists','crimenotes','suspects','investigationcolse')); 
    }

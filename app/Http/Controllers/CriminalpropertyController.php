...
{
    protected $criminalfinancialService;

    public function __construct(CriminaFinanceService $criminalfinancialService)
    {
        $this->criminalfinancialService = $criminalfinancialService;
    ...
    public function showpropertycriminal(Request $request)
    {
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function($row) {
                    ...
                    if (auth()->user()->can('Financial-Crime_details-Add')) {
                    ...
                    $btn .= '<a href="' . route('criminalpropertyview', ['id' => $row->id]) . '"  target="_self" title="View" data-bs-toggle="tooltip" data-bs-placement="top"  class="icon-button btn btn-sm mr-1 btn-info editbtn"><i class="material-icons">visibility</i></a>';

                    if (auth()->user()->can('Financial-Crime_Judgement-Add')) {
        ...
        return view('Criminals.Criminal_Property.criminalproperty');
    }

    public function store(Request $request,  CriminaFinanceService $criminalfinancialService)
    {
        $message = $criminalfinancialService->store($request);
        return redirect()->back()->with('message', $message);
    }
    
    public function crimeverdict (Request $request,  CriminaFinanceService $criminalfinancialService)
    {
        $message = $criminalfinancialService->crimeverdict($request);
        return redirect()->back()->with('message', $message);
    }
    ...
    public function update(Request $request ,  CriminaFinanceService $criminalfinancialService)
    {
        $message = $criminalfinancialService->update($request);
        return redirect()->back()->with('message', $message);
    }

    public function updateotherCrimeVerdict(Request $request ,  CriminaFinanceService $criminalfinancialService)
    {
        $message = $criminalfinancialService->updateCrimeVerdict($request);
        return redirect()->back()->with('message', $message);
    }
